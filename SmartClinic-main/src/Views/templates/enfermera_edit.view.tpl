<div class="container section-pad">
  <div class="form-card">
    <div style="margin-bottom:30px;">
      <h2 style="color:#033B9F;margin-bottom:10px;font-size:2.2rem;">Editar Enfermera</h2>
      <p style="color:#636366;">Actualiza los datos profesionales, lugares de atención y la cuenta de acceso vinculada.</p>
    </div>
    ...
    <form method="POST" action="index.php?page=EnfermerasController&action=edit&id={{id}}">
      ...
      <fieldset style="margin:28px 0 0;padding:0;border:0;">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:14px;">
          <span style="color:#111827;font-size:1.15rem;font-weight:700;">Centros de salud y área/turno</span>
        ...
        <div style="border:1px solid #D1D5DB;border-radius:8px;overflow:hidden;">
          ...
          <div data-centro-assignment data-centro-search="{{nombre}} {{codigo}} {{ciudad}}" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:18px;align-items:center;padding:16px;border-bottom:1px solid #E5E7EB;">
            ...
            <div class="form-group" style="margin:0;">
              <label for="area_{{id}}">Área / turno</label>
              <input data-consultorio id="area_{{id}}" type="text" name="areas[{{id}}]" maxlength="50" value="{{area}}" {{if selected}}required{{endif selected}} {{ifnot selected}}disabled{{endifnot selected}}>
            </div>
          </div>
          {{endfor centros}}
        </div>
      ...
      <div class="form-actions">
        <a href="index.php?page=EnfermerasController&action=index" class="btn btn--outline">Cancelar</a>
        {{if puedeGuardar}}
        <button type="submit" class="btn btn--primary">Actualizar enfermera</button>
        {{endif puedeGuardar}}
      </div>
    </form>
  </div>
...
document.querySelectorAll("[data-centro-assignment]").forEach(function (row) {
  ...
  var area = row.querySelector("[data-consultorio]");

  function syncArea() {
    area.disabled = !checkbox.checked;
    area.required = checkbox.checked;
    if (!checkbox.checked) {
      area.value = "";
    }
  }

  checkbox.addEventListener("change", syncArea);
  syncArea();
});
