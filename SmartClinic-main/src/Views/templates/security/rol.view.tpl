...
<div class="form-card">
  ...
  <form method="post" action="index.php?page=Security_Rol">
    ...
    <div class="form-field">
      <label>Código</label>
      <input type="text" name="rolescod" value="{{rolescod}}" class="form-input" {{if field_readonly}}readonly{{endif field_readonly}} autocomplete="off">
      {{if rolescod_error}}<div class="error">{{rolescod_error}}</div>{{endif rolescod_error}}
    </div>

    <div class="form-field">
      <label>Descripción</label>
      <input type="text" name="rolesdsc" value="{{rolesdsc}}" class="form-input" {{if field_readonly}}readonly{{endif field_readonly}} autocomplete="off">
      {{if rolesdsc_error}}<div class="error">{{rolesdsc_error}}</div>{{endif rolesdsc_error}}
    </div>

    <div class="form-field">
      <label>Estado</label>
      {{if field_readonly}}
      <select name="rolesest" class="form-select" disabled>
      {{endif field_readonly}}
      {{ifnot field_readonly}}
      <select name="rolesest" class="form-select">
      {{endifnot field_readonly}}
        <option value="ACT" {{rolesest_act}}>Activo</option>
        <option value="INA" {{rolesest_ina}}>Inactivo</option>
      </select>
      {{if rolesest_error}}<div class="error">{{rolesest_error}}</div>{{endif rolesest_error}}
    </div>
    ...
    <div class="form-actions">
      {{if show_commit}}
        ...
        {{endifnot is_delete}}
      {{endif show_commit}}
      <a href="index.php?page=Security_Roles" class="btn-cancelar">Cancelar</a>
    </div>
  </form>
</div>